...
void retrieveLastColor() {
  ...
  lastG = EEPROM.read(1);
  lastB = EEPROM.read(2);
  globalBrightness = EEPROM.read(3) / 255.0;  // Restore brightness as a float (0.0 - 1.0)
}
...
void loop() {
    ...
    // Periodically fetch schedules to update IDs
    static unsigned long lastFetch = 0;
    unsigned long now = millis();
    if (now - lastFetch > 60000) {  // Every 60 seconds
        fetchSchedulesFromAdafruitIO();
        lastFetch = now;
    }
}
